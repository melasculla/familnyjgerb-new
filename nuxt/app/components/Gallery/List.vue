...
const openImage = (image: typeof items.value[0]) => {
   activeImageRef.value = image
...
const activeImage = computed(() => {
   ...
   const prev = () => {
      // if (index <= 3 && currentPage.value > 1 && status.value !== 'pending')
   ...
   const next = () => {
      if (index >= (items.value.length - 2) && currentPage.value < pages.value && status.value !== 'pending') {
         addMore.value = true
         currentPage.value = currentPage.value + 1
      }
...
const galleryNav = (e: KeyboardEvent) => {
   if (e.key === 'ArrowLeft' && activeImage.value?.prev !== null) openImage(activeImage.value!.prev)
   if (e.key === 'ArrowRight' && activeImage.value?.next !== null) openImage(activeImage.value!.next)
}
</script>
...
<template>
   <div class="">
      ...
      <div
         class="grid grid-rows-[3.1rem_1fr_3.1rem] px-5 [--_border:var(--color-basic-900)] *:border-[var(--_border)] transition-all"
         ...
         *:p-3 *:block *:leading-none uppercase font-semibold">
         ...
         <GalleryFilters class="col-[1/1] row-[2/-1] sticky top-14 h-[94svh] w-full border-r pt-5 filters"
            :gallery="type" />

         <div v-for="item in 2" :class="item === 1 ? 'row-[1/1] border-b' : 'row-[3/3] col-[2/-1]'"
            class="content-center grid grid-cols-[1fr_3fr_1fr] items-center gap-2 overflow-hidden px-5">
            <div class="flex items-center gap-2 cursor-pointer" @click="showFilters = !showFilters">
               <svg class="h-6 transition-all duration-500" :class="{ 'rotate-180': !showFilters }" viewBox="0 0 16 16"
                  fill="none">
                  <path
         ...
         <div class="col-[2/-1] row-[2/2] px-5 py-7 border-b">
            <div v-if="addMore ? true : (status !== 'error' && status !== 'pending')">
                     ...
                     <Teleport to="#teleports">
                        <div v-if="isOpen && activeImage"
                           ...
                           <div class="size-full" @click="closeImage()">
                              <NuxtImg :src="activeImage.image" placeholder="/loader.svg"
                           ...
                           <button v-if="activeImage.prev !== null" @click="openImage(activeImage.prev)"
                              class="absolute max-xl:bottom-10 xl:top-1/2 xl:-translate-y-1/2 left-2 lg:left-20 bg-accent-300 rounded-xl lg:rounded-3xl cursor-pointer">
                              <svg class="size-14 lg:size-20 text-white" width="32" height="32" viewBox="0 0 24 24">
                                 <path fill="currentColor"
                                    d="m7.85 13l2.85 2.85q.3.3.288.7t-.288.7q-.3.3-.712.313t-.713-.288L4.7 12.7q-.3-.3-.3-.7t.3-.7l4.575-4.575q.3-.3.713-.287t.712.312q.275.3.288.7t-.288.7L7.85 11H19q.425 0 .713.288T20 12t-.288.713T19 13z" />
                              </svg>
                           </button>

                           <button v-if="activeImage.next !== null" @click="openImage(activeImage.next)"
                              class="absolute max-xl:bottom-10 xl:top-1/2 xl:-translate-y-1/2 right-2 lg:right-20 bg-accent-300 rounded-xl lg:rounded-3xl cursor-pointer">
                              <svg class="size-14 lg:size-20 text-white" width="32" height="32" viewBox="0 0 24 24">
                                 <path fill="currentColor"
